...
Suppression: `.gitleaks.toml` extends Gitleaks defaults with three narrow entries: the `test/fixtures/secret-scan/` path (the controlled canary), a `generic-api-key`-only rule allowlist for `test/` paths (test idempotency scaffolding values are not credentials; vendor-specific rules still apply everywhere), and one historic commit SHA whose docs example self-matched the generic rule before it was reworded (kept in history since force-push is forbidden; real leaks must still be revoked/rotated first, never just allowlisted). Add new suppressions as narrow path/regex/commit entries with a comment citing the reason and issue; never blanket-exclude source dirs or file types.
