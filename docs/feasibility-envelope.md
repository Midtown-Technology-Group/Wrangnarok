...
Dated: 2026-09-18. Baseline: upstream `gobifrost/bifrost@3543c7ebee0e1bd9a2cab6dfba080a30621b1c5f` vs Wrangnarok `origin/main@9cc00ed` (lane `lane/177-sync-slice`; provider allowances re-checked 2026-09-17, unchanged; Worker script-size row corrected 2026-09-19 on `lane/177-limits-reconcile`, measured bytes unchanged).

<!-- LIMITS-META {"budgetKiB":730,"measuredBytes":975852,"measuredDate":"2026-09-18","minHeadroomBytes":8192} -->
...
| Worker script size | 64 MiB max uncompressed, both plans (no compressed-size limit) | [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) (2026-09-04 revision) |
...
Correction 2026-09-19 (`lane/177-limits-reconcile`, issue #177): the Worker script-size row previously claimed a 3 MB cap with a 10 MB paid-tier variant. Per Cloudflare [Workers limits](https://developers.cloudflare.com/workers/platform/limits/) (2026-09-04 revision) the cap is **64 MiB uncompressed on both plans** with no compressed-size limit; `Total Upload` in `wrangler deploy --dry-run` is the size. Every remaining stale hard-cap / hard-ceiling claim in this matrix is corrected to the same figure (the Pocketflare note below already stated it). The 730 KiB advisory soft reference is unchanged.
...
- Worker bundle: **975,852 bytes raw** measured 2026-09-18 via `npm run check:bundle` on this lane (`lane/177-sync-slice` sync at `origin/main@9cc00ed`; CI number governs) against a **730 KiB** advisory soft reference level (see reference levels below). The run exits zero: the soft reference is an early warning, never a merge gate. Delta vs the last META record (800,067 B): +175,785 B (~171.7 KiB), of which +15,959 B landed after the 2026-09-18 scout reading (959,893 B) via merges #525 (RUN-02 child-saga grant gate), #526/#524 (TRG-03 emissions, OAuth lifecycle), and #527 (EMBED-01 slice 3 session uploads). Top metafile contributors: `src/index.ts` 182,384 B (18.8%), `src/forms.ts` 56,091, `src/sdk.ts` 45,097, `src/orgs.ts` 35,516, `src/ops.ts` 32,643 (66 inputs, all `src/*`); no new runtime dependencies. The 228,332 B advisory overrun is recorded here deliberately per the supervision decision accepting warn-only overruns (issue #177 policy), not silently absorbed — any reference raise stays a #177 policy decision with shrink-before-raise first. The provider hard cap is 64 MiB uncompressed (~1.5% consumed, ~63.1 MiB of provider headroom remains) — enforced by Wrangler at deploy time, not duplicated locally.
- Client JS: 410,492 bytes raw / 112,373 B gzip (measured 2026-09-18 via `npm run build:ui` on this lane; `index-14PWmEuZ.js`) + 12,315 B CSS. Served as Static Assets from the same Worker; asset requests are free and unlimited per Workers pricing. Static Assets consume 0 Worker script bytes (Worker-side frontend code is only the `env.ASSETS.fetch` pass-through plus the asset-security shim).

### Soft-budget reference levels (advisory, not a gate)

Per the owner-approved policy decision (issue #177), the repository Worker bundle budget is advisory, not a merge gate. `scripts/check-bundle-budget.mjs` measures the exact bundle and prints bytes, headroom, and prominent warnings, but always exits zero for soft-threshold conditions: exceeding the `BUDGET_BYTES` reference level (730 KiB), falling below the `MIN_HEADROOM_BYTES` advisory reserve (8 KiB), or stale `LIMITS-META` bookkeeping. The script still fails on a real build/dry-run error, a malformed invocation, or an inability to obtain the artifact. `BUDGET_BYTES` / `MIN_HEADROOM_BYTES` drift versus `LIMITS-META` prints a warning so prose cannot silently trail code — without blocking the merge. The soft reference stays separate from Cloudflare's 64 MiB uncompressed hard deploy ceiling (2026-09-04 revision, both plans; enforced by Wrangler, never duplicated locally) and from billing limits.
...
| Workers bundle | 64 MiB uncompressed provider hard cap (both plans, 2026-09-04); 730 KiB advisory soft reference + 8 KiB advisory reserve | 975,852 bytes raw vs 747,520 | Soft reference is warn-only by design; 228,332 B over the reference on this lane (sync at `origin/main@9cc00ed`; +175,785 B vs the last 800,067 B META record, hand-written domain breadth across #525/#526/#524/#527). Growth is deliberate per-lane headroom under the accepted advisory overrun (recorded, not silently absorbed); no new runtime dependencies. |
...
| Full-stack UI as Static Assets (ADR 008) | free | Served from the same Worker; client JS 410 kB raw / 112 kB gzip; asset requests free. |
...
| Build/CI costs (Vite UI, workerd test matrix) | free | Local `vite build` plus GitHub-hosted CI minutes; no Cloudflare build product is adopted. Advisory soft reference (730 KiB + 8 KiB reserve) tracks deploy size well under the 64 MiB uncompressed provider cap. |
...
- **Pocketflare (#256):** publishes an ~8.9 MiB gzip deployed bundle over a 3 MB Free gzip gate that no longer exists — current [limits](https://developers.cloudflare.com/workers/platform/limits/) (updated 2026-09-05) enforce **64 MiB uncompressed on both plans** with no compressed-size limit (`Total Upload` in `wrangler deploy --dry-run` is the size; the gzip value is reference only). Its "$5 Paid floor" conclusion likely still holds in practice, but for CPU-time reasons, not bundle-gzip reasons: Free allows 10 ms CPU per invocation, and a Go-WASM-scale cold boot almost certainly exceeds it [inference from the documented WASM boot cost and Pocketflare's singleton-runtime design — not measured, per #256]. Lesson for us: our 975,852-byte raw bundle (≈ 0.93 MB) clears the uncompressed cap with huge margin — bundle bytes are not our Free constraint. Our Free story must be argued in measured CPU-ms and cold starts, not bundle bytes (see the test-only `Server-Timing` follow-up in #256). Our own soft reference stays advisory early warning, never the binding constraint.
...
2. `npm run build:ui` then `npm run check:bundle`: the deploy envelope is observed — exact bytes, headroom, and warnings print against the 730 KiB advisory reference and the 8 KiB advisory reserve, and the run exits zero for soft-threshold conditions (over reference, low reserve, or stale `LIMITS-META`). Only a build/dry-run error, malformed invocation, or missing artifact fails. Record reference-level changes deliberately with the reason noted.
3. `npx vitest run test/limits-envelope.test.ts`: the matrix covers every required capability row, names the evidence class of each number, carries a current Durable Objects row, and carries a truthful `LIMITS-META` block (shape-pinned; exact agreement with the script is advisory) — all without credentials or deployment.
