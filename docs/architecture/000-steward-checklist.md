...
- 2026-09-17: bundle-budget advisory policy per #177 (owner decision). One-diagram paths unchanged (no primitive, tenancy, or persistence change; measurement only). The 730 KiB threshold and 8 KiB reserve are now advisory reference levels: `npm run check:bundle` stays in CI as an observational check printing bytes/headroom/warnings and exiting zero for soft-threshold conditions; only build/dry-run errors, malformed invocations, or missing artifacts fail. LIMITS-META consistency is advisory reporting; parse/shape tests still guard truthful docs. No second authoritative path; deployment validity stays with Wrangler's own 3 MB enforcement.
