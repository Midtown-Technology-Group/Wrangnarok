...
- 2026-09-17: bundle-budget advisory policy per #177 (owner decision). One-diagram paths unchanged (no primitive, tenancy, or persistence change; measurement only). The 730 KiB threshold and 8 KiB reserve are now advisory reference levels: `npm run check:bundle` stays in CI as an observational check printing bytes/headroom/warnings and exiting zero for soft-threshold conditions; only build/dry-run errors, malformed invocations, or missing artifacts fail. LIMITS-META consistency is advisory reporting; parse/shape tests still guard truthful docs. No second authoritative path; deployment validity stays with Wrangler's own 64 MiB uncompressed enforcement.
- 2026-09-17: TRG-03 S3a operator retry/replay (issue #139, Phase 4 gate). One-diagram verdict: PASS, single path each. Authentication/authorization: operator writes via requireManageOrg, member-open reads, dispatch-time revalidation through the canonical resolver/grant path (shared with fan-out). Execution: retry dispatches through the standard submit protocol with identical `evt-` keys via the shared single-subscriber helper — one fence sequence, one key derivation. Persistence: no new migration; failed set derived from the S2 receipt rows. Secrets/deployment: untouched. Recovery: same-key replay convergence, the existing mechanism. No second authoritative path; no new primitive; Free-tier neutral; lane touches only its scope.
- 2026-09-19: LIMITS-01 provider-number reconcile per #177 (`lane/177-limits-reconcile`). One-diagram verdict: PASS, single path each. Docs-only correction of the stale "3 MB max (Paid: 10 MB)" Worker script-size claim to Cloudflare's 2026-09-04 64 MiB uncompressed limit (both plans); 730 KiB advisory soft reference unchanged; no primitive, tenancy, persistence, or deployment change. No second authoritative path; lane touches only its scope.
