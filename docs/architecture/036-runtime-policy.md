...
Every Execution snapshots the effective policy into `executions.policy_json` at submit. The Workflow resolves step retry limits and vendor deadlines through that snapshot, never the live operator row: in-flight runs keep the behavior they started with when an operator edits policy mid-flight. Detail exposes the snapshot under `policy`; missing snapshots (pre-migration rows) report the code default, never an invented per-Saga guess. Execution-time policy reads are fail-closed through the single `loadExecutionPolicy` loader (Slice A, issue #135, PR #433): a legacy `policy_json NULL` row resolves to the code default, but a D1 read failure propagates and no Saga/vendor work runs under invented defaults. The same holds for the admission active-count query, so a blind D1 fault can never over-admit concurrent Executions.

Authorization follows the authoritative Organization membership boundary: any authenticated caller in the Organization may inspect policy (`GET /api/sagas/:id/policy`), while `PUT` requires an active Organization-admin membership or instance-admin identity through `requireManageOrg`. Ordinary members may read policy but cannot change it (`403 ADMIN_ONLY`), even if they supply forged operator headers. Unknown Saga IDs answer 404, never an existence leak across tenants. The `PUT` gate stays `requireManageOrg` until AUTH-02 lands its narrow shape (admin/operator/viewer org roles plus explicit scoped delegation grants, deny-by-absence); that lane re-gates the writer without changing the read posture or the snapshot invariant.
...
| Crash / recovery | Dispatch stays `createBatch` retained-ID dedup plus the durable `dispatched` marker with caller-driven retry. The 15-minute same-revision refusal gate is unchanged; `409 RECOVERY_EXPIRED` never resurrects and never invents success. Policy snapshots ride the same row, so recovery replays under the original behavior. Slice C (issue #135, PR #439): terminal detail converges from authoritative D1 with advisory `runtimeStatus: null` when native Workflow history/status is missing — no Cron/sweeper, no resurrection, fences intact. |
| Execution-policy fail-closed (Slice A) | A forced policy-snapshot D1 read failure runs zero vendor work under defaults; recovery replays under the original non-default snapshot. Submit-time policy/count read failures refuse dispatch (`500`, no receipt mutation) instead of over-admitting. Legacy `NULL` snapshots still resolve to code defaults. |
| Long-wait / no-timeout lifetime (Slice B, open) | `timeout.vendorTimeoutMs = 0` is a vendor-call deadline ("use the Integration default"), not upstream's no-execution-timeout contract. Long-running execution lifetime and scoped-authority retention stay undecided; no JWT copy, no stepTimeout retuning here. |
