...
4. PR standard: open the PR ready-for-review, keep the required `Validate` check green, resolve every review thread, then enable GitHub's Merge when ready / merge-queue control. Do not use Mergify comments or labels.
...
8. Rebase ONLY on reported conflict, never for currency. The native merge queue re-validates the PR against predicted `main` via the `merge_group` event, so an "out of date" PR still queues and merges; every currency update burns a full fresh CI cycle and resets queue position.
